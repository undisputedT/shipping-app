...
const Dashboard: React.FC = () => {
  ...
  useEffect(() => {
    // Simulating API call to fetch user's shipments
  ...
  return (
    <div className="container mx-auto px-4 py-12">
      <h1 className="text-3xl font-bold mb-8">Your Shipments</h1>
            ...
            >
              {shipments.map((shipment) => (
                <div key={shipment.id} className="p-4 border-b last:border-b-0">
                  ...
                  <p>
                    <strong>Insurance:</strong> {shipment.insurance ? "Yes" : "No"}
                  </p>
                  <p>
                    <strong>Special Instructions:</strong> {shipment.specialInstructions || "None"}
                  </p>
                  <p>
          ...
          <tbody className="bg-white divide-y divide-gray-200">
            {shipments.map((shipment) => (
              <tr key={shipment.id}>
                ...
                <td className="px-6 py-4 whitespace-nowrap">{shipment.shipmentType}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.shippingMethod}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.weight} kg</td>
                <td className="px-6 py-4 whitespace-nowrap">
                  {shipment.dimensions.length}x{shipment.dimensions.width}x{shipment.dimensions.height} cm
                </td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.fragile ? "Yes" : "No"}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.insurance ? "Yes" : "No"}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.specialInstructions || "None"}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.senderName}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.receiverName}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.status}</td>
                <td className="px-6 py-4 whitespace-nowrap">{shipment.estimatedDelivery}</td>
              </tr>
            ))}
          </tbody>
        </table>
